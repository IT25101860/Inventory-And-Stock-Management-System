package com.inventory.management.productmanagement.controller;
...
public class ProductController {
    ...
    public String create(@Valid @ModelAttribute Product product,
        ...
        if (productService.isSkuTaken(product.getSku(), null))
            result.rejectValue("sku", "duplicate", "This SKU is already in use.");
        if (result.hasErrors()) {
            model.addAttribute("categories", Product.Category.values());
            model.addAttribute("isEdit", false);
            model.addAttribute("activePage", "products");
    ...
    public String update(@PathVariable Long id,
        ...
        if (productService.isSkuTaken(product.getSku(), id))
            result.rejectValue("sku", "duplicate", "SKU already used by another product.");
        if (result.hasErrors()) {
            model.addAttribute("categories", Product.Category.values());
            model.addAttribute("isEdit", true);
            model.addAttribute("activePage", "products");
